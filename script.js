...
function cifrar(texto, deslocamento) {
  let textoMaiuscula = texto.toUpperCase().split("");
  let textoCriptografado = [];

  for (let i = 0; i < textoMaiuscula.length; i++) {
    let indiceDaLetra = alfabeto.indexOf(textoMaiuscula[i]);
    if (indiceDaLetra >= 0) {
      textoCriptografado.push(letraPorIndice(indiceDaLetra + deslocamento));
    } else {
      textoCriptografado.push(textoMaiuscula[i]);
    }
  }
